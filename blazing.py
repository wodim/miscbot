...
bots = [(telegram.Bot(token), threading.Semaphore()) for token in _config_list('blazing_craiyon_tokens')]
persistence_semaphore = threading.Semaphore()
...
def blazing_craiyon(i) -> None:
    ...
    while True:
        ...
        try:
            new_prompt, _ = sub_translate(prompt, scramble_languages)
            if len(new_prompt) > len(prompt) * .5:
                # make sure the scrambler didn't oversimplify it
        ...
        bot, bot_semaphore = random.choice(bots)
        with bot_semaphore:
            while True:
                try:
